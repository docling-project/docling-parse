...
async def test_async_parallel_page_loading():
    ...
    pdf_doc: PdfDocument = await parser.load_async(
        path_or_stream=filename, lazy=True, boundary_type=PdfPageBoundaryType.CROP_BOX
    )
    ...
    try:
        ...
        print(
            "WARNING: Parallel loading succeeded - this may indicate thread-safety has been fixed"
        )
        ...
        print(
            "The C++ implementation needs proper synchronization for concurrent page parsing"
        )
