...
def _log_text_cell(page_no: int, text: str, bbox) -> None:
    print(
        f"{page_no}\t{text!r}\t{_format_bbox_coord(bbox.l)}\t"
        f"{_format_bbox_coord(bbox.t)}\t{_format_bbox_coord(bbox.r)}\t"
...
def export(input_path: Path, mode: str, scale: float, threads: int,
           page: int | None = None, log_text: bool = False) -> DoclingDocument:
