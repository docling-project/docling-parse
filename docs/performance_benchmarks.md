...
<!-- Generated by perf/run_scaling.py; see docs/performance_benchmarks/ for the full per-machine reports. -->
...
The companion CSV holds one row per `(backend, task, threads, document, page)`, including the rendered `image_width` and `image_height`. It is the input for the render size check and, via `perf/run_eval.py`, for every plot shown above.
...
The document and page counts are printed by any `run_scaling.py` invocation. The distributions are produced by `perf/run_eval.py` from the per-page CSV, which carries the per-page time and rendered canvas size for each backend and task alongside the document key:

```sh
uv run python ./perf/run_eval.py \
