...
namespace pdflib
{
  ...
  public:
    ...
    // The font's code -> UTF-8 map (from /ToUnicode, a CMap resource, or
    // identity). Exposed so subset siblings of the same base font can share.
    const cmap_value& get_cmap() const { return cmap_numb_to_char; }
    void merge_cmap_from(const cmap_value& other) { cmap_numb_to_char.merge_missing(other); }

    // True for a predefined CMap whose codespace is two bytes wide and whose
    // codes ARE Unicode: the UCS2 and UTF16 families (UniJIS-UCS2-H,
    // UniGB-UCS2-H, UniKS-UCS2-H, ...). Such a code must never be split into
    // single bytes -- 0x3057 is し, not '0' followed by 'W'.
    bool uses_two_byte_cmap() const
    {
      if(encoding != CMAP_RESOURCES) { return false; }
      return encoding_name.find("UCS2")  != std::string::npos or
             encoding_name.find("UTF16") != std::string::npos;
    }

    // True for /Subtype /Type3, whose glyphs are content-stream procedures
    // rather than a font program. Substituting a system face for such a font
    // is meaningless: the character codes index procedures, not glyphs.
  ...
  void pdf_resource<PAGE_FONT>::init_default_width()
  {
	...
	has_default_width = true;
      ...
      {
	// ISO 32000-1 table 117: for a CID font, a missing /DW means 1000 --
	// one full em, the advance of a full-width CJK glyph. Falling through
	// to the historical 500 halved every unlisted advance, overlapping
	// full-width text by 50%.
	has_default_width = true;
	default_width = 1000;
        LOG_S(INFO) << "no /DW on CID font: using the spec default " << default_width;
      }
    else
      {
	default_width = 500;
